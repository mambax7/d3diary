...
// **		**
	...
	$sql_tmp_base = "FROM ".$xoopsDB->prefix($mydirname.'_diary')." d 
			...
			WHERE ".$whr_openarea.$whr_time.$whr_tag.$whr_nofuture." AND ".$whr_uids." 
			 ORDER BY ".$odr ;
	...
	*/
	...
	while ( $dbdat = $xoopsDB->fetchArray($result) ) {
		...
		$_entry['openarea']=$_tmp_openarea;
		$openarea_cat = intval($dbdat['openarea_cat']);
		if ($openarea_cat>0) { $_entry['openarea'] = $openarea_cat; }
		if (intval($dbdat['openarea'])>0) { $_entry['openarea'] = intval($dbdat['openarea']); }
	...
	if ( $dosort == true && (empty($b_tag) || $mod_config['use_tag']==0) ) {
		// at 1st getting uids and cids 
		// *********** SQL for
		// temprary ids other enrties
